...
Our results are summatised in \autoref{fig:accOverEpochs}, \autoref{fig:accOverNoiseP} and \autoref{tbl:noiseresults}. We observe a steady accuracy decline with rising noise. At first the difference is only small but with larger noise probability the accuracy drops drasticly. With these results it is save to say that the network accualy uses the context information to implycitly model the dialog state.
...
In our ablation study we found that removing (noising) the context from the training data does decrease the performance of the network. This shows that neural networks do indeed learn to implicitly model the dialog state and context.
